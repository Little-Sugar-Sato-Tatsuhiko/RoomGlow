...
| POST     | `/api/location/detect` | IPアドレスから緯度・経度を再検出して設定に反映 |
...
- 管理画面の「緯度」「経度」で観測地点を設定する。初回起動時は [ipapi.co](https://ipapi.co/)（APIキー不要）でサーバーのIPアドレスから自動検出する（失敗時は東京の座標にフォールバック）。管理画面の「現在地を自動検出」ボタンでいつでも再検出できる。緯度・経度を直接編集すると以降は自動検出されなくなる（`locationSource` 設定が `manual` になる）。
